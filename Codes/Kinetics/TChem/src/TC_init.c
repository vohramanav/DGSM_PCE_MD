...
int TC_initChem(char *mechfile,char *thermofile, int tab, double delT)
{
  ...
  */
  ...
  for ( i=0 ; i< TC_Nspec_ ;i++ )
  {
    ...
    if ( TC_Tlo_[i] > TC_TthrmMin_ ) TC_TthrmMin_ = TC_Tlo_[i];
    if ( TC_Thi_[i] < TC_TthrmMax_ ) TC_TthrmMax_ = TC_Thi_[i];
  }
  ...
  if ( TC_Nreac_ > 0)
  {
    ...
    TC_isDup_  = (int *) malloc( TC_Nreac_ * sizeof(int) ) ;
    for ( i=0 ; i< TC_Nreac_ ; i++ ) {
      fscanf( chemfile,"%d", &(TC_isDup_[i]) ) ;
    }
  ...
  if ( TC_nRealNuReac_ > 0 )
    for ( ir = 0 ; ir < TC_nRealNuReac_ ; ir++ )
    {
      ...
      /* products */
      //indx = ir*TC_maxSpecInReac_+TC_maxSpecInReac_/2 ;  // Bugfix Cosmin 2016/12/28
      indx = ir*TC_maxSpecInReac_+TC_reacNreac_[irR] ;
  ...
  if ( TC_nRealNuReac_ > 0 )
  {
    ...
    for ( j  = 0 ; j  < TC_nRealNuReac_        ; j++ )
    {
      ...
      //printf("got here...........\n");fflush(stdout);
      /* products */
      //indxR = j *TC_maxSpecInReac_+TC_maxSpecInReac_/2 ;  // Bugfix Cosmin 2016/12/28
      indxR = j *TC_maxSpecInReac_+TC_reacNreac_[jR] ;
  ...
  if ( TC_nRealNuReac_ > 0 )
  {
    ...
    for ( i=0 ; i<TC_nRealNuReac_ ; i++ )
    {
      for ( j = 0 ; j<TC_maxSpecInReac_ ; j++ )
      {
    	printf("%f ",TC_reacRealNuki_[i*TC_maxSpecInReac_+j]) ;
      }
      printf("\n") ;
    }
...
int TC_outChem(char *mechfile) {
  ...
  */
  ...
  int i, j, kspec, indx, indxR, irev, ithb, iord, ipfal, iplog, irstc;

  /* Retrieve things from kmod.list */
  FILE *outfile = fopen(mechfile,"w") ;

  /* ELEMENTS */
  fprintf(outfile,"ELEMENTS\n") ;
  for ( i=0 ; i<TC_Nelem_ ; i++ )
    fprintf(outfile,"%s\n", &(TC_eNames_[i*LENGTHOFELEMNAME]) ) ;
  fprintf(outfile,"END\n") ;
  fflush(outfile) ;
  ...
  if ( TC_Nreac_ > 0)
  {
    ...
    for ( i=0 ; i<TC_Nreac_ ; i++ ) {
      ...
      char rstr[100];
      TC_getReacStr(i, 100, rstr);
      fprintf(outfile,"%s",rstr);

      /* Write Arrhenius terms */
      indx = i*3;
      fprintf(outfile,"%16.8e\t%16.8e\t%16.8e\n",
	      TC_reacArhenFor_[indx], TC_reacArhenFor_[indx+1], TC_reacArhenFor_[indx+2]*RUNIV/CALJO) ;

      /* Write duplicate keyword */
      if ( TC_isDup_[i] == 1 ) {
        fprintf(outfile,"DUPLICATE\n");
      }

      /* Check if reverse Arrhenius terms given */
      if ( irev < TC_nRevReac_) {
        if ( TC_reacRev_[irev] == i ) {
          indx = irev*3;
          fprintf(outfile,"REV /%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacArhenRev_[indx],TC_reacArhenRev_[indx+1],TC_reacArhenRev_[indx+2]*RUNIV/CALJO) ;
          irev++;
	}
      } /* Done check if REV */

      /* Check if reaction has third-body factors */
      if ( ithb < TC_nThbReac_) {
        if (TC_reacTbdy_[ithb] == i ) {
          for ( j = 0 ; j<TC_reacTbno_[ithb] ; j++ ) {
            indx  = ithb*TC_maxTbInReac_+j ;
            kspec = TC_specTbdIdx_[indx] ;
            fprintf(outfile,"%s /%6.3f/ ", &(TC_sNames_[kspec*LENGTHOFSPECNAME]),TC_specTbdEff_[indx]) ;
          } /* done loop over efficiencies */
	  if (TC_reacTbno_[ithb]>0) fprintf(outfile,"\n");
          ithb++;
	}
      } /* done if reaction had third-body efficiencies */
        ...
        if (TC_reacAOrd_[iord] == i) {
          for ( j = 0; j<TC_maxOrdPar_ ; j++){
            ...
            if (TC_specAOidx_[indx]<0)
            {
              ...
              fprintf(outfile,"FORD /%s %16.8e/\n", &(TC_sNames_[kspec*LENGTHOFSPECNAME]),TC_specAOval_[indx]) ;
            }
            else if (TC_specAOidx_[indx]>0)
            {
              kspec = TC_specAOidx_[indx]-1 ;
              fprintf(outfile,"RORD /%s %16.8e/\n", &(TC_sNames_[kspec*LENGTHOFSPECNAME]),TC_specAOval_[indx]) ;
	    }
	  }
          iord++;
        }

      } /* done if reaction had arbitrary reaction orders */

      if ( ipfal < TC_nFallReac_ ) {
        if (TC_reacPfal_[ipfal] == i) {
          if (TC_reacPlohi_[ipfal] == 0) { /* LOW */
            indx = ipfal * TC_nFallPar_ ;
            fprintf(outfile,"LOW /%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacPpar_[indx], TC_reacPpar_[indx+1], TC_reacPpar_[indx+2]*RUNIV/CALJO) ;
	  } else { /* HIGH */
            indx = ipfal * TC_nFallPar_ ;
            fprintf(outfile,"HIGH /%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacPpar_[indx], TC_reacPpar_[indx+1], TC_reacPpar_[indx+2]*RUNIV/CALJO) ;
	  }
	  if ( TC_reacPtype_[ipfal] == 2 ) { /* SRI */
            indx += 3 ;
            fprintf(outfile,"SRI /%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacPpar_[indx], TC_reacPpar_[indx+1], TC_reacPpar_[indx+2]) ;
	  } else if ( TC_reacPtype_[ipfal] == 3 ) { /* TROE-3 */
            indx += 3 ;
            fprintf(outfile,"TROE /%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacPpar_[indx], TC_reacPpar_[indx+1], TC_reacPpar_[indx+2]) ;
	  } else if ( TC_reacPtype_[ipfal] == 4 ){ /* TROE-4 */
            indx += 3 ;
            fprintf(outfile,"TROE /%16.8e\t%16.8e\t%16.8e\t%16.8e/\n",
	      TC_reacPpar_[indx], TC_reacPpar_[indx+1], TC_reacPpar_[indx+2], TC_reacPpar_[indx+3]) ;
	  }
          ipfal++;
	}
      } /* done if reaction is fall-off */

      /* Reactions with PLOG formulation */
      if (TC_getReacPLOG(i)) {
        for (j=TC_reacPlogPno_[iplog]; j < TC_reacPlogPno_[iplog+1]; j++) {
          double *api = &(TC_reacPlogPars_[4*j]);
	  fprintf(outfile,"PLOG /%f\t%16.8e\t%16.8e\t%16.8e/\n",
                  exp(*api),exp(*(api+1)),*(api+2),*(api+3)*RUNIV/CALJO);
	}
        iplog++;
      }
