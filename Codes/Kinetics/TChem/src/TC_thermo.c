...
int TC_getCpSpecMlFcn(double t,double *cpi)
{
  ...
  tLoc = t;
  if ( tLoc < TC_TthrmMin_ ) {
    tLoc = TC_TthrmMin_;
  }
  if ( tLoc > TC_TthrmMax_ ) {
    tLoc = TC_TthrmMax_;
  }
  ...
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    ...
    if ( i9t < TC_nNASA9coef_ ) {
      if ( i == TC_spec9t_[i9t] ) {
        TC_getCpFcn9t(tLoc, i9t, &cpi[i]) ; i9t++ ;
        continue ;
      }
    }

    ipol = 0 ;
    if (tLoc > TC_Tmi_[i]) ipol = 1 ;
    icpst = i*7*2+ipol*7 ;
    cpi[i] = TC_cppol_[icpst]+tLoc*(TC_cppol_[icpst+1]+
			                        tLoc*(TC_cppol_[icpst+2]+
				                      tLoc*(TC_cppol_[icpst+3]+
				                      tLoc* TC_cppol_[icpst+4]
				                           )
				                           )
				                           ) ;
  }
...
int TC_getCpSpecMl1Fcn(double t, int i, double *cpi)
{
  ...
  tLoc = t;
  if ( tLoc < TC_TthrmMin_ ) tLoc = TC_TthrmMin_;
  if ( tLoc > TC_TthrmMax_ ) tLoc = TC_TthrmMax_;
  ...
  if (tLoc > TC_Tmi_[i]) ipol = 1 ;
  icpst = i*7*2+ipol*7 ;
  *cpi = TC_cppol_[icpst]+tLoc*(TC_cppol_[icpst+1]+
                          tLoc*(TC_cppol_[icpst+2]+
                          tLoc*(TC_cppol_[icpst+3]+
                          tLoc* TC_cppol_[icpst+4]
                               )
...
int TC_getCpSpecMsPFcn(double t,double *cpi)
{
  ...
  if (fabs(delT)>REACBALANCE) {
    for ( i = 0 ; i < TC_Nspec_ ; i++ ) cpi[i] = 0.0 ;
    return (0);
  }
  ...
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    ...
    if ( i9t < TC_nNASA9coef_ )
    {
      if ( i == TC_spec9t_[i9t] )
      {
        TC_getCpFcnP9t(tLoc, i9t, &cpi[i]) ; i9t++ ;
        continue ;
      }
    }
    ...
    if (tLoc > TC_Tmi_[i]) ipol = 1 ;
    icpst = i*7*2+ipol*7 ;
    cpi[i] = TC_cppol_[icpst+1]+tLoc*(2.0*TC_cppol_[icpst+2]
                               +tLoc*(3.0*TC_cppol_[icpst+3]
                               +tLoc* 4.0*TC_cppol_[icpst+4]
		                                 )
		                                 ) ;
  }
...
int TC_getCpSpecMlPFcn(double t,double *cpi)
{
...
int TC_getCpSpecMs1PFcn(double t, int i, double *cpi)
{
  ...
  if (tLoc > TC_Tmi_[i]) ipol = 1 ;
  icpst = i*7*2+ipol*7 ;
  *cpi = TC_cppol_[icpst+1]+tLoc*(2.0*TC_cppol_[icpst+2]+
			                      tLoc*(3.0*TC_cppol_[icpst+3]+
				                    tLoc* 4.0*TC_cppol_[icpst+4]
				                         )
				                         ) ;
  *cpi *= TC_Runiv_/TC_sMass_[i] ;
  ...
  if (fabs(delT)>REACBALANCE) { *cpi = 0.0; }
...
int TC_getHspecMlFcn(double t,double *hi)
{
  ...
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    ...
    if ( i9t < TC_nNASA9coef_ )
    {
      if ( i == TC_spec9t_[i9t] )
      {
        TC_getHspecFcn9t(tLoc, i9t, &hi[i]) ; i9t++ ;
        continue ;
      }
    }
    ...
    if (tLoc > TC_Tmi_[i]) ipol = 1 ;
    icpst = i*7*2+ipol*7 ;
    hi[i] = tLoc*(TC_cppol_[icpst  ]+
            tLoc*(TC_cppol_[icpst+1]*one2+
            tLoc*(TC_cppol_[icpst+2]*one3+
            tLoc*(TC_cppol_[icpst+3]*one4+
            tLoc*(TC_cppol_[icpst+4]*one5
                 )
                 )
                 )
                 )
  ...
  if (fabs(delT)>REACBALANCE) {
    TC_getCpSpecMlFcn(tLoc,TC_cpks);
    for ( i = 0 ; i < TC_Nspec_ ; i++ ) hi[i] += TC_cpks[i]*delT;
  }
...
/************************************************************************************
     ___       _       _____
    |_ _|_ __ | |_    | ____|_ __   ___ _ __ __ _ _   _
     | || '_ \| __|   |  _| | '_ \ / _ \ '__/ _` | | | |
     | || | | | |_ _  | |___| | | |  __/ | | (_| | |_| |
    |___|_| |_|\__(_) |_____|_| |_|\___|_|  \__, |\__, |
                                            |___/ |___/
************************************************************************************/
/**
 * \ingroup thermo
...
int TC_getEntr0SpecMlFcn(double t,double *s0i) {
  ...
  for ( i = 0 ; i < TC_Nspec_ ; i++ )
  {
    ...
    if (tLoc > TC_Tmi_[i]) ipol = 1 ;
    icpst = i*7*2+ipol*7 ;
    s0i[i] = TC_cppol_[icpst]*tln+tLoc*(TC_cppol_[icpst+1]+
                                  tLoc*(TC_cppol_[icpst+2]*one2+
                                  tLoc*(TC_cppol_[icpst+3]*one3+
                                  tLoc* TC_cppol_[icpst+4]*one4
                                       )
                                       )
                                       ) + TC_cppol_[icpst+6];
  }
  ...
  if (fabs(delT)>REACBALANCE) {
    TC_getCpSpecMlFcn(tLoc,TC_cpks);
    for ( i = 0 ; i < TC_Nspec_ ; i++ ) s0i [i] += TC_cpks[i]*log(t/tLoc);
  }
...
int TC_getEntr0SpecMsPFcn(double t,double *ei)
{

  int i ;
  TC_getEntr0SpecMlPFcn(t, ei);
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) ei[i] *= TC_Runiv_/TC_sMass_[i] ;

  return ( 0 ) ;

}

int TC_getEntr0SpecMlPFcn(double t,double *ei)
{
  ...
  if (fabs(delT)>REACBALANCE) {
    TC_getCpSpecMsFcn(tLoc,TC_cpks);
    for ( i = 0 ; i < TC_Nspec_ ; i++ ) ei[i] = TC_cpks[i]/tLoc ;
    return (0);
  }
  ...
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    ...
    if (tLoc > TC_Tmi_[i]) ipol = 1 ;
    icpst = i*7*2+ipol*7 ;
    ei[i] = TC_cppol_[icpst]/tLoc+
            TC_cppol_[icpst+1]+tLoc*(TC_cppol_[icpst+2]
                              +tLoc*(TC_cppol_[icpst+3]
                              +tLoc* TC_cppol_[icpst+4]
                                    )
                                    ) ;
  }
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) ei[i] *= TC_Runiv_ ;
