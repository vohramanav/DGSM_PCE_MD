 ...
 * \brief Returns the index of reaction in the list of reactions with real stoichiometric coeffs
*/
int TC_getReacArbOrdIdx(int ireac) { 
/**
   \param ireac : reaction index
*/
...
*/
  ...
  if ( TC_nRealNuReac_ > 0 )
  {
	...
	indxR = ir              * TC_maxSpecInReac_+TC_reacNreac_[i] ; // Bugfix Cosmin 2016/12/28
	for ( j = 0; j<TC_reacNprod_[i] ; j++) {
	  kspec = TC_reacSidx_[indx] ;
	  stoicoef[kspec] += TC_reacRealNuki_[indxR] ;
	  indx++  ;
...
int TC_getReacStr(int ireac, int lenstr, char *rstr) {
  ...
  int ir, i, j, indx, kspec, ifall, iflg1 ;
  char tmpstr[100];
  ...
  for ( j = 0 ; j<TC_reacNreac_[ireac] ; j++) {
    ...
    if ( ir >= 0 ) {
      ...
      if ( fabs(TC_reacRealNuki_[indx]) != 0.0 ) {
        iflg1=1;
	if (j != 0) {
          sprintf(tmpstr,"%s","+") ; 
          strcat(rstr, tmpstr) ;
      ...
      if ( iflg1 && fabs(TC_reacRealNuki_[indx]+1) != 0.0 ) {
        sprintf(tmpstr,"%5.2f",-TC_reacRealNuki_[indx]) ; 
        strcat(rstr, tmpstr) ;
        memset(tmpstr, 0, 100) ; 
      }
    }
    ...
    if (iflg1) {
      ...
      strcat(rstr, tmpstr) ;
      memset(tmpstr, 0, 100) ; 
      //if (j != TC_reacNreac_[ireac]-1) sprintf(tmpstr,"%s","+") ;  
      strcat(rstr, tmpstr) ;
      memset(tmpstr, 0, 100) ; 
    }
  }
  ...
  for ( j = 0 ; j<TC_reacNprod_[ireac] ; j++) {
    iflg1=0;
    if ( ir >= 0 ) {
      indx = ir*TC_maxSpecInReac_+TC_reacNreac_[ireac]+j;
      if ( fabs(TC_reacRealNuki_[indx]) != 0.0 ) {
        iflg1=1;
	if (j != 0) {
          sprintf(tmpstr,"%s","+") ; 
