...
int tcgetarhenpresdepfor_( int *ireac, int *ipos, double *val )
  {
    int ans = 0 ;
    ans = TC_getArhenPresDepFor( *ireac, *ipos, val ) ;
    return ( ans );
  }
...
int tcgetjacdiagtynanl_(double *scal, int *Nspec, double *jac)
{
  int ans = 0 ;
  ans = TC_getJacDiagTYNanl(scal,(*Nspec),jac) ;
  return ( ans ) ;
}
