...
#ifdef DEBUGMSG
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
     printf("TC_getReacRates (omega): %d -> %e\n",i+1,omega[i]);
  }
#endif
...
int TC_getgkFcn(double t1,double t_1, double tln) {
  ...
  TC_getEntr0SpecMlFcn(t1,TC_gk );
  TC_getHspecMlFcn    (t1,TC_hks);
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    TC_gk[i] = -tln+(TC_gk[i]-TC_hks[i]*t_1)/TC_Runiv_;
    //printf("%e ",TC_gk[i]);
  }
  //printf("\n");
  return ( 0 ) ;
...
int TC_getgkpFcn(double t1,double t_1, double tln)
{
  ...
  TC_getEntr0SpecMlPFcn(t1,TC_gkp );
  TC_getHspecMlFcn     (t1,TC_hks);
  TC_getCpSpecMlPFcn   (t1,TC_cpks);
  for ( i = 0 ; i < TC_Nspec_ ; i++ ) {
    TC_gkp[i] = -t_1+(TC_gkp[i]+TC_hks[i]*t_1*t_1-TC_cpks[i]*t_1)/TC_Runiv_;
  }
    ...
    if ( TC_isRev_[i] )
    {
      ...
      if ( irev < TC_nRevReac_)
      {
        if ( TC_reacRev_[irev] == i )
        {
          /* yes, reverse Arhenius parameters are given */
          indx = irev*3 ;
	  if (TC_reacArhenRev_[indx]<1.0e-14)
            TC_krev[i] = 0.0;
          else
      ...
      else
      {
        ...
        kc         = TC_kc_coeff[i]*exp(sumNuGk) ;
        TC_krev[i] = TC_kfor[i]/kc ;
	//printf("%d: %e %e %e %e\n", i, TC_kfor[i], kc, TC_kc_coeff[i], sumNuGk);
  ...
  for ( i = 0 ; i<TC_Nreac_; i++)
  {
    ...
    if ( irnu < TC_nRealNuReac_ )
      if ( TC_reacRnu_[irnu] == i )
      {
...
#ifdef NONNEG
          TC_ropFor[i] *= pow(fabs(concX[kspec]),niup) ;
#else
        ...
        {
          indx  = i   *TC_maxSpecInReac_+TC_maxSpecInReac_/2 ;
          //indxR = irnu*TC_maxSpecInReac_+TC_maxSpecInReac_/2 ;  // Bugfix Cosmin 2016/12/28
          /* compute reverse rop */
          for ( j = 0; j<TC_reacNprod_[i] ; j++) {
            double nius ;
            kspec = TC_reacSidx_[indx] ;
...
#ifdef NONNEG
            TC_ropRev[i] *= pow(fabs(concX[kspec]),nius) ;
#else
    ...
    if ( iord < TC_nOrdReac_ )
      if (TC_reacAOrd_[iord] == i)
      {
	    ...
	    double niup = TC_specAOval_[indx];
#ifdef NONNEG
            TC_ropFor[i] *= pow(fabs(concX[kspec]),niup) ;
#else            
            TC_ropFor[i] *= pow(concX[kspec],niup) ;
          ...
          {
            kspec = TC_specAOidx_[indx]-1 ;
	    double nius = TC_specAOval_[indx];
#ifdef NONNEG
            TC_ropRev[i] *= pow(fabs(concX[kspec]),nius);
#else
            TC_ropRev[i] *= pow(concX[kspec],nius);
