...
% Input data
X = dlmread(fullfile('pts_pce3D_3D.dat')) ;
Y = dlmread(fullfile('record_id_pce3D.dat')) ;
MetaOpts.ExpDesign.X = X;
MetaOpts.ExpDesign.Y = Y;

L = [1.7235e14 5.5530e19 1.082e17];
U = [2.1065e14 6.7870e19 1.322e17];

nom = [1.915e14 6.170e19 1.202e17];
...
pts3D = zeros(nsams,3);
pts3D(:,1) = pts(:,1); pts3D(:,2) = pts(:,9);
pts3D(:,3) = pts(:,15);

Y_PCE = uq_evalModel(pts3D);
Y_Model = qoi;
...
pts_PCE = 0.95.*min(Y_PCE):step:1.05.*max(Y_PCE);
...
set(gcf,'color',[1,1,1]);
leg = legend({'$\mathrm{Model~(19D)}$','$\mathrm{PCE~(3D)}$'});
set(leg,'Interpreter','latex');
box on;
grid on;
print -depsc pdf_comp_lean.eps
end
