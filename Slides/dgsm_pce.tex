...
\section{\scshape MD}
\subsection{md}
\begin{frame}{Phonon Transport in Silicon}
...
\begin{itemize}
\myitem Global sensitivity analysis (GSA) of potential field parameters.
\vspace{2mm}
...
\begin{itemize}
\myitem Both, GSA and the Forward Problem are computationally intractable.
\vspace{2mm}
\myitem Explore the applicability of DGSM to construct a reduced-order
surrogate.   
\end{itemize}
...
%
\subsection{md}
\begin{frame}{NEMD in a Silicon Bar}

\begin{center}
%%        \animategraphics[autoplay,poster=first,loop,height=0.60\textwidth]{5}
%%        {./animation/snap}{0}{64}
\begin{figure}[htbp]
        \includegraphics[width=0.5\textwidth]{./Figures/snap0}
\end{figure}
\end{center}
...
\node (x7_f) [x7,below of=case_f,yshift=0.2cm] {$\gamma$: 0.3023};
\node (x1_f) [x1, below of=x7_f,yshift=0.2cm] {$A$: 0.2066};
\node (x5_f) [x5, below of=x1_f,yshift=0.2cm] {$\alpha$: 0.1795};
\node (x4_f) [x4, below of=x5_f,yshift=0.2cm] {$q$: 0.1172};
\node (x3_f) [x3, below of=x4_f,yshift=0.2cm] {$p$: 0.1137};
\node (x6_f) [x6, below of=x3_f,yshift=0.2cm] {$\lambda$: 0.0438};
\node (x2_f) [x2, below of=x6_f,yshift=0.2cm] {$B$: 0.0369};
\node (pool) [pool, below of=x5_f,yshift=-1.0cm]{};

\node (case_10) [outer,right of=case_5,xshift=2.5cm]{$\hat{\mathcal{C}_i\mu_i}$:
10 Samples};
\node (x7_10) [x7, right of=x7_f,xshift=2.5cm] {$\gamma$: 0.2537};
\node (x5_10) [x5, right of=x1_f,xshift=2.5cm] {$\alpha$: 0.2176};
\node (x1_10) [x1, right of=x5_f,xshift=2.5cm] {$A$: 0.2008};
\node (x2_10) [x2, right of=x4_f,xshift=2.5cm] {$B$: 0.0958};
\node (x3_10) [x3, right of=x3_f,xshift=2.5cm] {$p$: 0.0896};
\node (x4_10) [x4, right of=x6_f,xshift=2.5cm] {$q$: 0.0759};
\node (x6_10) [x6, right of=x2_f,xshift=2.5cm] {$\lambda$: 0.0666};
\node (pool) [pool, below of=x1_10,yshift=-1.0cm]{};
