...
estimated analytically using a sparse-basis PCE. Note that estimating $\mathcal{T}(\theta_i)$
using a PCE has been shown to be more efficient compared to sampling techniques.
Moreover, a sparse-basis PCE is expected to require fewer training points which
enhances computational savings. To investigate the impact of the dimensionality, the
...
33 uncertain parameters wherein the activation energies~($E_{a,i}$'s) are also considered to
be uncertain. The $E_{a,i}$'s are also considered to be uniformly distributed in the
...
$E_{a,i}$'s with a non-zero nominal value are considered as uncertain, and therfore the
...
space are considered to have converged once $\epsilon_\text{\tiny LOO}$ is found to be
...
and the 33D cases are found to be 60 and 500 respectively. As expected, the predictive
...
Sensitivity estimates for the 33 uncertain parameters are hence determined
using the DGSM-based strategy and by evaluating $\mathcal{T}(\theta_i)$ analytically
using the sparse-basis PCE as discussed earlier in this section. The estimates based
on the two approaches were plotted separately and placed side-by-side for comparison
...
\caption{
Left: Estimates of the screening metric ($\widehat{\mathcal{C}_i\mu_i}$) for each 
uncertain parameter, obtained using $N$ = 5, 10, 15, and 20 samples in the full parameter space.
Right: Sobol' total sensitivity index, $\mathcal{T}(\theta_i)$ for uncertain parameters in the
semilinear elliptic PDE~\eqref{eq:semilinear}.}
\label{fig:sense33D}
\end{center}
\end{figure}
%
