...
\multicolumn{1}{ |l|| }{\textbf{DGSM-based}, $\widehat{\mathcal{C}_i\mu_i}$} & 5(19+1) = 100 & 5(33+1) = 170 \\ 
\cline{1-3}
\multicolumn{1}{ |l|| }{\textbf{Total-effect Sobol'}, $\mathcal{T}(\theta_i)$} & 20 + $N_v$ & 
500 + $N_v$ \\ \cline{1-3}  
\end{tabular}
\end{center}
\label{tab:cost}
...
including for gradient computation using finite difference. Whereas,
constructing the sparse PCE itself requires model runs at 500 training points to sufficiently converge. Additional
model runs are required to further verify the accuracy of the resulting PCE. Note that the analysis was performed
...
analysis also reveals the significant increase in computational effort pertaining to the estimation of the total-effect
...
parameters, conventional approaches can quickly become prohibitive and the proposed framework could help
