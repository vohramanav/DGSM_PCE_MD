...
\alennote{Are all these reactions, reversible? The notation $\Longleftrightarrow$ suggests
that. Also, 
for reversible reactions, I prefer the notation $\rightleftharpoons$.}
...
\ee
%
The numerator in the right hand side represents the observed (obs) fuel-oxygen mass ratio at a given condition
(\alennote{What does the subscript \emph{obs} stand for?}) 
and the denominator represents the stoichiometric (st) ratio of the same quantity. Hence,
...
~\alennote{What is the rationale behind such small numbers? These are smaller
than the unit round-off.} 
...
\alennote{This is hardly surprising, five samples in a 19D parameter space
is going to get us so far; I suggest removing or rewording this statement, 
as a careful referee will jump on this.}
...
\alennote{I don't understand; did we not sample the system to death to get 
total Sobol' indices, or we are saying that's too difficult, and 
we just use DGSMs and our screening approach?}
However,
...
\end{figure}
%
Clearly, the ROS captures the spread as well as the modal estimate of
