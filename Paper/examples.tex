...
We consider the following semi-linear elliptic PDE on a 2D square domain ([0,1]$\times$[0,1]):
...
\end{equation}
%
where $u$ is referred to as the velocity, $\kappa$ and $c$ are coefficients of the diffusion term
and the non-linear term in the above equation respectively. The velocity, $u$ is random since the
source term, $q$ in RHS is modelled as a truncated summation with random coefficients:

\be
q = \sum\limits_{i=1}^{N=8}\alpha_i\sin\left(\frac{i\pi x}{8}\right)\cos\left(\frac{i\pi y}{8}\right)
\label{eq:source}
\ee
%
Hence, $u~=~u(\vec{\theta})$ with $\vec{\theta} = (\kappa,c,\alpha_1,\alpha_2,\ldots,\alpha_{8})^T$, a vector
of uncertain parameters. Table~\ref{tab:elliptic} provides distributions of the uncertain
input parameters. The solution of~\eqref{eq:semilinear} for
a particular set of values of the uncertain parameters is illustrated below in Figure~\ref{fig:usoln}. 

\begin{table}[htbp]\label{tab:elliptic}
\renewcommand*{\arraystretch}{1.2}
\begin{center}
\begin{tabular}{cl}
\toprule
...
\end{tabular}
\end{center}
\caption{Description and distributions of uncertain parameters in the 
semi-linear elliptic PDE in~\eqref{eq:semilinear}.}
\end{table}

\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=0.6\textwidth]{./Figures/u_soln}
  \caption{Plotted above is the solution, $u(x,y)$ of~\eqref{eq:semilinear} at 
  of $\kappa$~=~0.075, $c$~=~1.5, and $\alpha_i$~=~4.0.}
\label{fig:usoln}
\end{center}
\end{figure}

We aim to construct a reduced-order surrogate for the following quantity of interest (QoI):
...
\ee
%
where $D$ is the region: $[2/5, 3/5] \times [2/5, 3/5]$. Once again, the availability of
cheap model evaluations allow us to obtain converged estimates of the Sobol
total-effect index ($\mathcal{T}(\theta_i)$), plotted in Figure~\ref{fig:sense_elliptic}.
...
\caption{
Left: Sobol' total sensitivity index, $\mathcal{T}(\theta_i)$ for uncertain parameters in the 
semi-linear elliptic PDE~\eqref{eq:semilinear}. Right: 
...
\end{figure}
%
Sensitivity predictions based on the screening metric, $\widehat{\mathcal{C}_i\mu_i}$ are found to
be in close agreement with $\mathcal{T}(\theta_i)$, even for the case when $N$ = 5. Moreover, 
the uncertainty in the QoI in \eqref{eq:qoi} is observed to be largely dependent on $c$, 
...
As expected, the ROS converges at a much faster rate. Using model evaluations at 90
sample points, $\epsilon_{\mbox{\tiny{LOO}}}$ is found to be two orders of magnitude
smaller than that in the case of full-surrogate ($\mathcal{O}(10^{-4}$) vs. $\mathcal{O}(10^{-2}$)). 
