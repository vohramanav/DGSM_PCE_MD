...
source term, $q$ in the RHS is modelled as a truncated summation with random coefficients:
...
\ee
%
Hence, $u~=~u(\vec{\theta})$ with $\vec{\theta} = 
(\kappa,c,\alpha_1,\alpha_2,\ldots,\alpha_{8})^T$, a vector
of uncertain parameters. Distributions of the uncertain
input parameters are tabulated below in Figure~\ref{fig:elliptic}~(left).
...
of the 2D semi-linear elliptic PDE~\eqref{eq:semilinear} using
...
%\begin{tabular}{cc}
%
%  \includegraphics[width=0.45\textwidth]{./Figures/u_soln.png} &
%\begin{tabular}{cl}
%\toprule
%$\theta_i$ & \textbf{Distribution} \\ 
%\bottomrule
...
\ee
%
where $D$ is the region: $[2/5, 3/5] \times [2/5, 3/5]$ in $\mathbb{R}^{x\times y}$. 
The availability of
cheap model evaluations allow us to obtain converged estimates of the Sobol
total-effect index ($\mathcal{T}(\theta_i)$), plotted in Figure~\ref{fig:sense_elliptic}~(left),
...
space. The L-2 norm was estimated to be 0.05 i.e. the ROS was found to be accurate
within 5$\%$. Additionally, we compare PDFs of the QoI, $\mathcal{F}(\kappa,c,\theta)$,
generated using the full-space and the reduced-space surrogates in 
Figure~\ref{fig:conv_elliptic}~(right). The two PDFs are observed to be in excellent 
agrement with each other. Hence, the ROS is successfully verified and could be
used in lieu of the full-space surrogate to reduce computational effort. 
