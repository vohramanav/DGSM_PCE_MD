...
\begin{tabular}{|l||l|}
\hline
\textbf{Parameter Description} & \textbf{Distribution} \\ \hline \hline
...
given by~\eqref{eq:bore}. $\mathcal{N}(\mu,\sigma^2)$ denotes a normal distribution with
mean, $\mu$ and variance, $\sigma^2$. $\mathcal{U}[a,b]$ denotes a uniform distribution
with limits $a$ and $b$.}
\label{tab:bore}
\end{table}
%
...
\caption{
Left:Left: Sobol' total sensitivity index, $\mathcal{T}(\theta_i)$ for uncertain parameters in the
...
In this test problem, we consider an undamped non-linear oscillator with only one degree of
freedom as illustrated below in Figure~\ref{fig:osc}. 

\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=0.6\textwidth]{./Figures/oscillator}
\caption{Schematic illustration of an undamped non-linear oscillator system with one degree of freedom.}
\label{fig:osc}
\end{center}
\end{figure}

The above system has been studied extensively for its reliability using
...
which either spring yields, $\bm{X}:~\{m,k_1,k_2,r,F,t_1\}$ is a set of uncertain parameters, and   
$\omega_0~=~\sqrt{\frac{k_1+k_2}{m}}$. The uncertain parameters are considered to be normally distributed with mean
...
\caption{Description and distributions of uncertain parameters in the limit state function
given by~\eqref{eq:limit}. $\mathcal{N}(\mu,\sigma^2)$ denotes a normal distribution with
mean, $\mu$ and variance, $\sigma^2$.}
\label{tab:osc}
\end{table}

Converged estimates of $\mathcal{T}(\theta_i)$ are once again obtained easily by
evaluating $g(\bm{X})$ for a large set of samples in $\mathbb{R}^{|\bm{X}|}$ as
shown in Figure~\ref{fig:sense_osc}~(left). Estimates of the screening metric, 
...
From the above figure, it can be said that the Sobol sensitivity indices are in excellent
agreement with the screening metric estimates. Consequently, the parameter ranks are
found to be consistent in the two plots. It is observed that the limit state function is
predominantly sensitive towards three parameters: $r$, $F$, and $t_1$. Hence, it seems
likely that an ROS in three dimensions is able to capture the uncertainty
in $g(\bm{X})$ due to uncertainty in the six parameters. To illustrate computational gains
as a result of dimension-reduction, we construct PCEs in 3D, 4D (accounts for the uncertainty
in $k_1$ as well), and the original 6D parameter domain. In Figure~\ref{fig:conv_osc}~(left), we
compare the rate of convergence of the PCEs. As expected, the third-order PCE converges much faster
than the other two. Consequently, the number of model evaluations required for a converged
PCE is expected to be relatively much smaller in this case.  
...
In order to verify the accuracy of the third-order PCE, we estimate the relative L-2 norm of the
difference between predictions of  $g(\bm{X})$, obtained using the actual function in ~\eqref{eq:limit}
and the PCE using~\eqref{eq:l2}.  The value of $\epsilon_{\mbox{\tiny{L-2}}}$ using evaluations at
100 points was found to be 0.0835 i.e. the third-order PCE is accurate within 8.36$\%$. Furthermore, 
the accuracy is assessed in a probabilistic sense by comparing PDFs
of $g(\bm{X})$, obtained using the function and the 3D PCE as illustrated in Figure~\ref{fig:conv_osc}~(right).
Clearly, the two PDFs are in excellent agreement with each other. Hence, the ROS constructed 
using the methodology presented in section~\ref{sec:method} could be used reliably to characterize
as well as quantify the uncertainty in $g(\bm{X})$ due to parametric uncertainty associated with
its functional description in~\eqref{eq:limit}, efficiently. 
