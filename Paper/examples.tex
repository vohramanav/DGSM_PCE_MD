...
was used to solve the resulting nonlinear system.~\alennote{Give the
number of degrees of freedom.} 
We computed converged estimates of the Sobol
total-effect index $\mathcal{T}(\theta_i)$, reported 
in Figure~\ref{fig:sense_elliptic}~(left).~\alennote{Say how many PDE solves
were used.}
\begin{figure}[htbp]
 \begin{center}
...
within 5$\%$. 
%The L-2 norm was estimated to be 0.05 i.e. 
...
effort. 
\alennote{Regarding the PDFs reported in figures 3, 6, 9: I think since we 
are sampling the model at a large number of points to compute the
total Sobol indices, we might as well save those sames, and then
use them to compute a KDE of the ``true'' PDF; that way, each of these
PDF comparison plots will report: true, FSS, ROS PDFs. I think that will
make the case 
for the presented method stronger, given we remain close to the true PDF.}
