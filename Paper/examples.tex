...
convergence trends as a function of training runs for the ROS with the full-space
...
\begin{figure}[htbp]
 \begin{center}
  ...
  \includegraphics[width=0.48\textwidth]{./Figures/ub_conv_borehole}
\caption{
Left: Sobol' total sensitivity index for uncertain parameters in the borehole
function. Right: 
estimates of the screening parameter ($\hat{\mathcal{C}_i\mu_i}$) is plotted
...
legend. \alennote{Can we have larger fontsizes for the right image? Also the y-axis label
needs to be modified with a widehat as done in the text now.}}
...
Figure~\ref{fig:conv_bore}, we compare convergence of PCE's constructed in the
...
used for constructing the PCE in 4, 5, and 7 dimensions. \alennote{Use different line styles
and colors. Also use thicker lines. Instead of taking log of the error, just use a 
semilogy in Matlab to use logarithmic scale on the y-axis.}}
\label{fig:conv_bore}
\end{center}
\end{figure}
...
space seems much higher.~\alennote{How are these samples used? Are we doing 
Latin Hypercube sample, or some other QMC approach?} 
...
Figure~\ref{fig:pdfcomp_bore} illustrates a comparison of the PDF's of the
discharge, $\mathcal{Q}$ obtained by propagating 10$^6$ Monte-Carlo (MC)
samples through the 7D PCE in the original input parameter domain as well as
the reduced order PCE's constructed in 4 and 5 dimensions. 
\begin{figure}[htbp]
 \begin{center}
...
\alennote{Use thicker lines, different colors in addition to line styles, larger 
font size, and also use the x-axis limits to be $[0, 300]$, maybe? 
Once figures 4, 5 are appropriate reformatted, I would combine them in one line as I did 
things in figure 2.}
} 
\label{fig:pdfcomp_bore}
\end{center}
\end{figure}
It is evident from this plot, that the PDF's agree quite favorably with each
other. Consequently, it can be said that the reduced order PCE is verified in a
