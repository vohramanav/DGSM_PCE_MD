    ...
    %  \State Draw samples $\{ \bm{\theta}_k \}_{k = 1}^{n_1}$ 
     ...
     %   \State $N_\text{total} = N_\text{total} + n_1$.
      \State Compute $\bm{g}^k = \nabla_{\bm{\theta}}G(\bm\theta_k)$, 
             $k = 1, \ldots, n_1$. 
\alennote{Should we have $n_1 = n_0$? Also, there is an issue with this 
algorithm: when you call this first time according to the flow diagram, you will
not actually have the gradients $\bm{g}^k$, $k = 1, \ldots, N_\text{total}$;
moreover, we need to explicitly augment the gradient samples with the ones 
passed to the method. Moreover, I think the step $N_\text{total} = N_\text{total} + n_0$
should be in the body of this algorithm, as opposed to where it is in the flow 
diagram.}
      \State Compute 
...
i}}$ is used.~\alennote{What is that modified formulation? Need to specify.}
...
\alennote{You mean to do L2 error study in 
addition to the leave-one-out error studies?}
...
the ROS predictions corresponding to a large number of samples (say, 10$^6$). 
However, in realistic problems involving complex, compute-intensive 
simulations, the PDF based on model evaluations would be intractable.
A practical alternative would be to compare a histogram
based on sparse model evaluations with the surrogate-based PDF
in order to gain some insight into the quality of the comparison. 
