...
model output.~\alennote{The next sentence seems unneeded, and 
might be confusing; consider removing it, or clarify that in 
the case of PCE, we construct PC bases as multivariate polynomials
on a parameter space with reduced dimension.}
...
~\alennote{Don't hard code section numbers, use labels, and refer to them.}
...
by the choice of maximum number of iterations, $s_\text{max}$. Key inputs
include a limiting value of the maximum relative change in the sensitivity
measure between succesive iterations $\tau$, a limiting ratio
$\tau_\text{screen}$ of the sensitivity metric relative to its maximum value,
a fraction $\beta$ to guide the number of new samples $ \lceil \beta n_1
\rceil$ denotes an integer $\leq~\beta n_1$ at each iteration, a set of
samples $\{ \bm{\theta}_k \}_{k = 1}^{n_0}$ for the initial screening step in
the algorithm, and the set of available gradient evaluations $\{
\bm{g}^k \}_{k=1}^{n_0}$. The outputs include the set of active
...
$\{ \bm{g}^k
\}_{k=1}^{N_\text{total}}$. Parameter screening is an integral part of the
overall framework, illustrated using a flow-diagram in Figure~\ref{fig:flow}.
...
\node (sam0) [process, below of=tol, text width=12.5em, yshift=-0.3cm] {Draw $n_0$ samples $\{ \bm{\theta}_k \}_{k = 1}^{n_0}$ 
...
\node (fss) [process, below of=counter, text width=14.5em, yshift=-0.28cm] {Construct regression-based 
surrogate in full-space (FSS) uing $N_\text{total}$ model evaluations};

\draw [arro] (counter) -- (fss);   
...
\node (ver) [process, left of=ros, text width=14.5em, xshift=-5cm] {Verify ROS using evaluations at $N_\text{total}$
...
%proposed framework depends upon the choice of the model output. 
%
Since the
screening metric involves computation of partial derivatives in the full
parameter space, the output must exhibit differentiable 
...
selected few. Hence, where possible, it is important to assess the nature of the
input-output relationship for a given model prior to implementing the present
framework. The set of required inputs for parameter screening are initialized,
...
\label{eq:alpha}
\ee
\alennote{I noticed in the latex code the use of the \textbf{noindent} command;
please remove those, we will use latext setting to formation paragraphs.} where
...
other hand, if $\alpha$ is considerably large (~\alennote{You mean close to
one?}), the set of inputs required for screening are updated as needed, and a
...
i}}$ is used. Accuracy of the surrogate could be assessed by evaluating the
...
which model evaluations are available. Accuracy of the surrogate could be
