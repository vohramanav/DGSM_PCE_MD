...
$s_\text{max}$, $\beta > 0$, $\{ \bm{\theta}_k \}_{k = 1}^{n_0}$. 
...
\node (val) [process, below of=start, text width=14.5em] {Construct a validation test suite using
...
\node (dtol) [io, above of=dr, text width=8.7em,align=left,yshift=3.2cm] {Update: $\tau$, $\tau_\text{screen}$, 
$s_\text{max}$, $\beta$};
