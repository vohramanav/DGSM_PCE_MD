...
\be
\mathcal{Y} = \sum_{\alpha\in\mathcal{I}} c_{\alpha}\Psi_{\alpha}(\bm{\xi(\theta)}) 
\ee
...
\be
\bm{c} = \mathbb{E}[\Psi_\alpha(\bm{\xi})\cdot\mathcal{Y}]
\approx
\sum_{i=1}^{N} \mathcal{Y}(\bm{\theta}^{(i)})w^{(i)}\Psi_\alpha(\bm{\xi}^{(i)})
\ee
...
\mathcal{M}(\bm{x})\right)^{2}\right]  + \lambda\normone{\bm{c}}
\ee
